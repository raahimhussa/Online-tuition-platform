...
export async function PATCH(req, { params }) {
    try {
      ...
      // Validate if the contract exists and is in 'pending' status
      const contractCheckQuery = `
        SELECT * 
        FROM hiring_contracts 
        WHERE contract_id = $1 AND teacher_id = $2 AND status = 'pending';
      `;
      const { rows: contractRows } = await query(contractCheckQuery, [contract_id, teacherId]);
  
      if (contractRows.length === 0) {
        return new Response(
          JSON.stringify({ message: 'Contract not found or not in pending status' }),
          { status: 404, headers: { 'Content-Type': 'application/json' } }
        );
      }
  
      // Update the status of the contract to 'rejected'
